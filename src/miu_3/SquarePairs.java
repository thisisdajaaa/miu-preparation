...
//	 -------------------|--------|-----------------------------------------------------------------------
    ...
    public static boolean isPerfectSquare(int n) {
        ...
        return (int) squared * (int) squared == n;
    }

    public static int getSquarePairsCount(int[] array) {
        if (array.length < 2) {
            return 0;
        }
        ...
        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < array.length; j++) {
                if (i != j) {
                    if (array[i] < array[j] && isPerfectSquare(array[i] + array[j]) && array[j] > 0 && array[i] > 0) {
                        tuples.put("outer: " + i + " inner: " + j, "<" + array[i] + "," + array[j] + ">");
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(getSquarePairsCount(new int[]{9}));
    }
