...
//	 -------------------|--------|-----------------------------------------------------------------------
    ...
    public static int isSquarePairs(int[] array) {
        if (array.length < 2) {
            return 0;
        }
        ...
        for (int i = 0; i < array.length; i++) {
            for (int j = 0; j < array.length; j++) {
                if (i != j && array[i] > 0 && array[j] > 0) {
                    if (array[i] < array[j] && isPerfectSquare(array[i] + array[j])) {
                        tuples.put("[" + i + ", " + j + "]", "<" + array[i] + "," + array[j] + ">");
                    }
                }
            }
        }
    ...
    public static void main(String[] args) {
        System.out.println(isSquarePairs(new int[] {11,5,4,20}));
    }
}
